...
resource "null_resource" "patch" {
  triggers = {
    ...
    cmd_patch  = "kubectl patch configmap/aws-auth --patch \"${local.aws_auth_configmap}\" -n kube-system --kubeconfig <(echo $KUBECONFIG | base64 --decode)"
  }
  ...
  provisioner "local-exec" {
    ...
    command = <<EOT
      ...
      source ~/.bashrc
      kubectl patch configmap/aws-auth --patch "${local.aws_auth_configmap}" -n kube-system --kubeconfig <(echo $KUBECONFIG | base64 --decode)
    EOT
  }
}